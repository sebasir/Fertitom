...
public class SimulationBean implements Serializable {
    ...
    private HashMap<Params, Property> paramValues;
    ...
    public void calcRecommendation(Params param, double value) {
        if (paramValues.get(param).getMinimumVal() > value || paramValues.get(param).getMaximumVal() < value) {
            showMessage("Cambio de Valor", "El valor introducido no esta en el rango válido", FacesMessage.SEVERITY_ERROR);
            return;
        }
        switch (param) {
            case NITROGEN:
    ...
    public void restart() {
        ...
        for (Property param : parameters) {
            paramValues.put(param.getParam(), param);
            switch (param.getParam()) {
                case NITROGEN:
                    simulation.setRecNitrogen(param.getDefaultVal());
                    break;
                case P2O5:
                    simulation.setRecPhosphorus(param.getDefaultVal());
                    break;
                case K2O:
                    simulation.setRecPotasium(param.getDefaultVal());
                    break;
            }
        }
        simDays = 0;
