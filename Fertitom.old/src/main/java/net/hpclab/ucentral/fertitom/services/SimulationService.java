...
public class SimulationService extends Thread implements Serializable {
    ...
    public void run() {
        ...
        try {
            ...
            eventBus.publish(Util.Constant.CHANNEL, json);
            String resultPath = Util.pathOutput + File.separator + simulation.getId() + File.separator + Util.Constant.REPORT_RESULT_FILE;
            System.out.println("Result HTML: " + resultPath);
            simulation.setHtmlResult(FileLoadService.getFileContent(resultPath));
            json.put("status", "renderHTML");
            json.put("htmlReport", simulation.getHtmlResult());
            eventBus.publish(Util.Constant.CHANNEL, json);
        } catch (Exception e) {
